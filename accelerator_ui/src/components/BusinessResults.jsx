...
const BusinessResults = ({ analysisResult, formData, onReset }) => {
  ...
  const fetchViolations = async (page, sessionIdOverride = null) => {
    ...
    try {
      ...
      if (res.ok) {
        // Update violations in the main result object
  ...
  const handleSaveRule = async (e) => {
    ...
    try {
      const rawPayload = {
        ...
        similarity_threshold: (newRule.rule_type === 'text_comparison' && newRule.pattern_type === 'fuzzy') 
          ? parseFloat(newRule.threshold_value) 
          : null,
      ...
      const dbColumns = [
        'rule_name', 'rule_type', 'rule_logic', 'description', 
        'source_column', 'target_column', 'threshold_value', 'operator', 
        'expected_value', 'pattern_type', 'min_value', 'max_value', 
        'severity', 'is_active', 'created_by', 'comparison_type', 
        'case_sensitive', 'kpi_type', 'aggregation', 'similarity_threshold'
      ];

      const filteredPayload = {};
      dbColumns.forEach(col => {
        if (rawPayload[col] !== undefined) {
          filteredPayload[col] = rawPayload[col] === "" ? null : rawPayload[col];
        }
      });
...
const handleExecuteRules = async () => {
    ...
    try {
      const payload = {
        ...
        // FIX: Add file_path at the top level so the backend can find it
        file_path: filePath, 
        source: {
          source_type: sourceType,
          file_path: filePath
      ...
      // Set initial result (Summary, Score, etc.)
      // We purposefully don't trust the violations array here for pagination 
      // reasons; we will fetch page 1 immediately.
      setExecutionResult({ 
        ...finalResult, 
        session_id: data.session_id, 
        violations: [] // Clear initially, let fetchViolations fill it
      });

      setStep('REPORT');

      // Immediately fetch Page 1 if we have a session ID
      if (data.session_id) {
        // Pass session_id explicitly because setExecutionResult is async/batched
        await fetchViolations(1, data.session_id);
      }

    } catch (err) {
      showNotify('error', 'Execution Failed');
      setLoading(false); // Stop loading if error
    } finally {
      // Note: Loading is turned off inside fetchViolations, 
      // but if we didn't call fetchViolations (e.g. no session_id), turn it off here.
      if (!executionResult?.session_id) { 
         // Logic check: if we failed before getting session ID, stop loading
         // We do this check loosely or just rely on the error catch above.
      }
    }
  };
          ...
          <div className="br-dynamic-section animate-fade-in">
            ...
            <div className="br-row-split">
              ...
              <div className="br-input-group">
                ...
                <select value={newRule.case_sensitive} onChange={e => setNewRule({ ...newRule, case_sensitive: e.target.value })}>
            ...
            {newRule.pattern_type === 'fuzzy' && (
              <div className="br-input-group">
                ...
                <input type="number" step="0.1" value={newRule.threshold_value} 
                  onChange={e => setNewRule({ ...newRule, threshold_value: e.target.value })} />
              </div>
            )}
          </div>
    ...
    return (
      <div className="br-results-container animate-fade-in">
        ...
        <div className="br-stats-grid">
          <div className="stat-card"><span>Total Rows</span><strong>{summary?.total_records_checked || '0'}</strong></div>
          <div className="stat-card fail"><span>Violations Found</span><strong>{summary?.total_violations || pagination.total}</strong></div>
        </div>
        ...
        <div className="br-violations-section">
          <div className="br-table-wrap">
            <table className="br-table">
              <thead>
                <tr><th>Row</th><th>Column</th><th>Rule</th><th>Actual Value</th><th>Comparison</th></tr>
              </thead>
              <tbody>
                {violations.length > 0 ? violations.map((v, i) => (
                  <tr key={i}>
                    <td>{v.excel_row || v.row_index}</td>
                    <td>{v.column_name}</td>
                    <td>{v.rule_name}</td>
                    <td>{String(v.actual_value ?? 'NULL')}</td>
                    <td>{v.comparison_type || v.expected_value}</td>
                  </tr>
                )) : (
                  <tr><td colSpan="5" style={{ textAlign: 'center', padding: '20px' }}>No violations found on this page.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {pagination.totalPages > 1 && (
            <div className="br-pagination-controls">
              ...
              <span className="br-page-info">
                ...
                <small style={{marginLeft: '8px'}}>({pagination.total} total)</small>
              </span>
              <button 
        ...
        <button className="br-reset-btn" onClick={onReset}>Return to Dashboard</button>
      </div>
    );
  };
